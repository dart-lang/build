...
- Added a `DebugIndexBuilder`, which by default generates a `test/index.html`
  ...
  tests.
- Fix `PackageAssetReader` when running with a package map generated by
  `build_runner` in a merged output directory. Drop support for a broken
  `findAssets` implementation.

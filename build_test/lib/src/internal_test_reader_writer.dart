...
class InternalTestReaderWriter extends ReaderWriter
  ...
  ///
  /// If provided [buildCachePackage] is the package where the build cache is
  /// written, otherwise `unset` is used.
  factory InternalTestReaderWriter({String? buildCachePackage}) {
    ...
    return InternalTestReaderWriter.using(
      ...
      assetPathProvider: InMemoryAssetPathProvider(
        buildCachePackage ?? 'unset',
      ),
      buildCachePackage: buildCachePackage ?? 'unset',
...
class InMemoryAssetPathProvider implements AssetPathProvider {
  ...
  InMemoryAssetPathProvider(this.buildCachePackage);
  ...
  String pathFor(
    ...
    if (hide) {
      id = AssetPathProvider.hide(id, buildCachePackage);
    }
    return id.toString();
  }
  ...
  bool isHiddenPath(String path) =>
      path.startsWith('$buildCachePackage|$generatedOutputDirectory');
}

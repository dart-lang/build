...
Future<bool> createMergedOutputDirectories(
  ...
  _log.progress(Progress.writeOutputDirectory);
  return await _log.attributeAsync(Attribution.write, () async {
...
Future<bool> _createMergedOutputDir(
  ...
  try {
    ...
    if (root != '' &&
      ...
      _log.severe('No assets exist in $root, skipping output');
      return false;
    }
  ...
  return _descriptorPool.withResource(() async {
    ...
    try {
      ...
      if (p.basename(id.path).startsWith('.')) {
        _log.fine('Skipping missing hidden file ${id.path}');
      } else {
        _log.severe(
...
Future<bool> _cleanUpOutputDir(
  ...
  if (!manifestFile.existsSync()) {
    if (outputDir.listSync(recursive: false).isNotEmpty) {
      ...
      switch (choice) {
        case 0:
          _log.severe('Skipped creation of the merged output directory.');
          return false;
        case 1:
          try {

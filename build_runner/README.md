...
> **NOTE**: Are you a user of this package? You may be interested in simplified
> user-facing documentation, such as our [getting started
> guide][getting-started-link].
...
- `--live-reload`: Enables automatic page reloading on rebuilds. Can't be used
...
should not be submitted to your source control repository. You can see
[our own `.gitignore`](https://github.com/dart-lang/build/blob/master/.gitignore)
as an example.
...
manually write one. With this approach every package which _uses_ a
[`Builder`][builder] must have it's own script, they cannot be reused from other
packages. A package which defines a [`Builder`][builder] may have an example you
can reference, but a unique script must be written for the consuming packages as
...
knowing how that change may have affected the outputs.
...
For the stability of the API and existing users, consider opening an issue first
before implementing a large new feature or breaking an API. For smaller changes
(like documentation, minor bug fixes), just send a pull request.

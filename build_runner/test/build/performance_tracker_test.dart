...
import 'package:build_runner/src/build/timing.dart';
...
void main() {
  group('PerformanceTracker', () {
    ...
    test('can track multiple phases', () async {
      ...
      final times = tracker.phases.map((t) => t.stopTime).toList();
      final expectedTimes = [5000, 10000, 15000].map(
      ...
      final total = tracker.phases.fold(
        const Duration(),
    ...
    test('can track multiple actions and phases within them, and '
      ...
      void checkMatchesExpected(BuildPerformance performance) {
        final allActions = performance.actions.toList();
        for (var i = 0; i < inputs.length; i++) {
          ...
          final allPhases = action.stages.toList();
          for (var p = 0; p < 3; p++) {
            final phase = allPhases[p];
        ...
        final total = performance.actions.fold(
          const Duration(),
      ...
      await scopedTrack(() async {
        for (final input in inputs) {
          final actionTracker = tracker.addBuilderAction(input, 'test_builder');

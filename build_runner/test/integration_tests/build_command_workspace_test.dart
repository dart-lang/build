...
void main() async {
  test('build command workspace', () async {
    ...
    // dep on build_runner or not
    // dep on which builders
    // what counts as root package
    // what counts as "all" package
    // builders mentioned in different packages
    // workspace dep onto builder
...
''');
    ...
    // dependency on `builder_pkg` via `p1`.
    tester.writePackage(
      name: 'p3',
...
''');
    ...
    // Builders do not apply.
    await tester.run('', 'dart run build_runner build --workspace');
    expect(tester.read('p3/lib/p3.txt.copy'), null);
    expect(
      tester.read('.dart_tool/build/generated/p2/lib/p2.txt.hidden'),
      null,
    );
...
''');
    ...
    // of `p1`.
    await tester.run('', 'dart run build_runner build --workspace');
    // `AutoApply.root` does not apply.
    expect(tester.read('p4/lib/p4.txt.copy'), null);
    // `AutoApply.all` does apply.

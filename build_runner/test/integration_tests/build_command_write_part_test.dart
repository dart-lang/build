...
''',
  ...
  test('resolving library in later phase sees generated part '
      'from earlier phase', () async {
    final pubspecs = await Pubspecs.load();
    final tester = BuildRunnerTester(pubspecs);
...
''',
    ...
    final output2 = await tester.run(
      'root_pkg',
      'dart run build_runner build --force-jit',
    );
    expect(output2, contains(BuildLog.successPattern));
  ...
  test(
    'librarySourceSink supports imports with enhanced-parts experiment',
    () async {
      final pubspecs = await Pubspecs.load();
      final tester = BuildRunnerTester(pubspecs);
...
''',
  ...
  test(
    'deleting source file deletes its generated part in incremental build',
    () async {
      final pubspecs = await Pubspecs.load();
      final tester = BuildRunnerTester(pubspecs);
...
''',
      ...
      tester.writePackage(
        ...
        dependencies: ['build_runner'],
        pathDependencies: ['write_part_pkg'],
        files: {'lib/a.dart': 'class A {}', 'lib/b.dart': 'class B {}'},
      );
      ...
      expect(
        tester.read('root_pkg/lib/_br_/b.dart'),
        contains('// part content'),
      );
      ...
      expect(output, contains(BuildLog.successPattern));
      expect(tester.read('root_pkg/lib/_br_/a.dart'), isNull);
      expect(
        tester.read('root_pkg/lib/_br_/b.dart'),
        contains('// part content'),
      );

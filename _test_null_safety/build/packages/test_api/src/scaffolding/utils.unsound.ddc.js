define(['dart_sdk', 'packages/test_api/src/backend/closed_exception'], (function load__packages__test_api__src__scaffolding__utils(dart_sdk, packages__test_api__src__backend__closed_exception) {
  ...
  var C = [void 0];
  var I = ["org-dartlang-app:///packages/test_api/src/scaffolding/utils.dart"];
  ...
  utils.pumpEventQueue = function pumpEventQueue(opts) {
    ...
    if (times == null) dart.nullFailed(I[0], 16, 28, "times");
    if (times === 0) return async.Future.value();
    return async.Future.new(dart.fn(() => utils.pumpEventQueue({times: dart.notNull(times) - 1}), T.VoidToFuture()));
  };
  utils.registerException = function registerException(error, stackTrace = C[0] || CT.C0) {
    if (error == null) dart.nullFailed(I[0], 23, 31, "error");
    if (stackTrace == null) dart.nullFailed(I[0], 24, 17, "stackTrace");
    async.Zone.current.handleUncaughtError(error, stackTrace);
  };
  utils.printOnFailure = function printOnFailure(message) {
    if (message == null) dart.nullFailed(I[0], 36, 28, "message");
    utils._currentInvoker.printOnFailure(message);
  };
  utils.markTestSkipped = function markTestSkipped(message) {
    let t0;
    if (message == null) dart.nullFailed(I[0], 45, 29, "message");
  ...
  }, '{"version":3,"sourceRoot":"","sources":["utils.dart"],"names":[],"mappings":";;;;;;;;;;;;;;;;;;;;;;;;;;;;QAe2B;;AACzB,QAAI,AAAM,KAAD,KAAI,GAAG,MAAc;AAE9B,UAAO,kBAAO,cAAM,6BAA4B,aAAN,KAAK,IAAG;EACpD;uDAG8B,OACd;;;AAGqC,IAA9C,AAAQ,uCAAoB,KAAK,EAAE,UAAU;EACpD;iDAQ2B;;AACc,IAAvC,AAAgB,qCAAe,OAAO;EACxC;mDAO4B;;;AAAY;;AAAiB,cAAK,OAAO;;;EAAC;;;;AAGlE,WAAQ;0BACP,WAAM,wBAAU,AACb,2EACA;IAA4B","file":"utils.unsound.ddc.js"}');
  // Exports:
  return {

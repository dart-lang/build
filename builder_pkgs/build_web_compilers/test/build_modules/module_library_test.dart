...
import 'package:build_web_compilers/src/build_modules/module_library.dart';
...
void main() {
  ...
  test('treats lib/src as non-entrypoint', () async {
    final library = ModuleLibrary.fromSource(
      makeAssetId('myapp|lib/src/a.dart'),
      '',
    );
  ...
  test('treats bin/executable as entrypoint', () async {
    final library = ModuleLibrary.fromSource(
      makeAssetId('myapp|bin/executable.dart'),
      'void main() {}',
    );
    expect(library.isEntryPoint, true);
  });
  ...
  test('can detect a main method', () async {
    ...
    expect(
      ModuleLibrary.fromSource(id, 'main(args, receivePort) {}').hasMain,
      true,
    );
  });
}

...
void main() {
  group('BuilderOptions', () {
    test('overrides with non-empty options', () {
      final defaults = const BuilderOptions({'foo': 'bar', 'baz': 'bop'});
      final overridden = defaults.overrideWith(
    ...
    test('overrides isRoot', () {
      final defaults = const BuilderOptions({}, isRoot: false);
      final overridden = defaults.overrideWith(BuilderOptions.forRoot);
      expect(overridden.isRoot, isTrue);
    });

    test('config doesnt change when overriding with empty options', () {
      final defaults = const BuilderOptions({'foo': 'bar', 'baz': 'bop'});
      final overridden = defaults.overrideWith(BuilderOptions.empty);
    ...
    test('changes nothing when overriding with null options', () {
      final defaults = const BuilderOptions({'foo': 'bar', 'baz': 'bop'});
      final overridden = defaults.overrideWith(null);
